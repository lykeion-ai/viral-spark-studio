...
export function ChatPanel({ onHighlightChange }: ChatPanelProps) {
  const { activePlatform, generatedContent, setGeneratedContent } = useApp();
  ...
  const renderContent = (content: string) => {
    ...
    return parts.map((part, index) => {
      ...
      if (tagColors[lowerPart]) {
        return (
          <span key={index} className={`font-semibold ${tagColors[lowerPart]}`}>
            {part}
          </span>
        );
      }
      ...
      <div className="p-4 border-t border-border">
        <div className="chat-input-wrapper !p-3">
          ...
          <button
            onClick={handleSend}
            disabled={!input.trim()}
            className="absolute right-3 top-1/2 -translate-y-1/2 w-9 h-9 bg-primary rounded-lg flex items-center justify-center text-primary-foreground hover:opacity-90 transition-all disabled:opacity-40"
          >
